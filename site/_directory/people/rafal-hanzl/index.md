---
type: person
slug: rafal-hanzl
name: Rafal Hanzl
title: Rafal Hanzl
position: Associate professor
department: Faculty of Film, TV and Games
image: /assets/images/portraits/Rafal_Hanzl_INN.jpg
institution: university-of-inland-norway
institutions:
- university-of-inland-norway
projects: []
roles:
- Member
urls:
  personal_website: https://creativetechnologist.eu
  institutional_website: ''
  github: https://github.com/rvrooom
  linkedin: https://www.linkedin.com/in/rafał-hanzl-992b0733/
  orcid: https://orcid.org/0000-0001-6316-2320
  nva: https://nva.sikt.no/research-profile/1300773
  youtube: ''
  mastodon: ''
  instagram: ''
  wikidata: ''
aliases: []
tags:
- Projections
- Multimedia Programs
- Computer Animation
- Interaction Design
- Artificial Intelligence
- Design, Art, Applied Art, Entrepreurship
- 3D Modelling
- Digital Photography
- Digital Twins
- Game Concept
- Game Design
- Compositing
search_keywords:
- Projections
- Multimedia Programs
- Computer Animation
- Interaction Design
- Artificial Intelligence
- Design, Art, Applied Art, Entrepreurship
- 3D Modelling
- Digital Photography
- Digital Twins
- Game Concept
- Game Design
- Compositing
selected_works:
- title: MishMash Opening Video Projection Installation
  year: '2026'
  source: Visual arts
  group_type: Visual arts
  contributors:
  - name: Marie Dahlén
    role: Artist
    slug: marie-dahlen
    url: /people/marie-dahlen/
  - name: Rafal Hanzl
    role: Artist
    slug: rafal-hanzl
    url: /people/rafal-hanzl/
  - name: Henning Birkeland
    role: Artist
    slug: henning-birkeland
    url: /people/henning-birkeland/
  - name: Jannicke Johansen
    role: Artist
    slug: jannicke-johansen
    url: /people/jannicke-johansen/
- title: Grønlandsport, sfinxen og «det nye lands bro»
  year: '2025'
  source: Visual arts
  group_type: Visual arts
  contributors:
  - name: Rafal Hanzl
    role: Artist
    slug: rafal-hanzl
    url: /people/rafal-hanzl/
  - name: Jannicke Johansen
    role: Artist
    slug: jannicke-johansen
    url: /people/jannicke-johansen/
- title: Technology and materiality/Shadow of War
  year: '2025'
  source: Film or video
  group_type: Film or video
  contributors:
  - name: Torkell Bernsen
    role: Director
  - name: Gustav Kvaal
    role: Director
  - name: Odd Torleiv Furnes
    role: SoundDesigner
    slug: odd-torleiv-furnes
    url: /people/odd-torleiv-furnes/
  - name: Rafal Hanzl
    role: VfxSupervisor
    slug: rafal-hanzl
    url: /people/rafal-hanzl/
  - name: Petter Østergaard
    role: RoleOther
  - name: Mikkel Wettre
    role: RoleOther
  - name: Marieke Verbiesen
    role: RoleOther
  - name: Taran Johanne Neckelmann
    role: RoleOther
- title: AKERSHUS 1624-1648 Video Installation
  year: '2025'
  source: Exhibition
  group_type: Exhibition
  url: https://akershus1648.extendedmedia.no
  contributors:
  - name: Rafal Hanzl
    role: ProjectLeader
    slug: rafal-hanzl
    url: /people/rafal-hanzl/
  - name: Maureen Thomas
    role: Writer
- title: Akershus 1624-1648 Interactive Exhibit
  year: '2024'
  source: Exhibition
  group_type: Exhibition
  url: https://akershus1648.extendedmedia.no
  contributors:
  - name: Rafal Hanzl
    role: ProjectLeader
    slug: rafal-hanzl
    url: /people/rafal-hanzl/
  - name: Maureen Thomas
    role: Writer
- title: Medieval Akersborg - Interactive Museum Exhibit
  year: '2023'
  source: Exhibition
  group_type: Exhibition
  url: https://medievalcastle.extendedmedia.no
  contributors:
  - name: Rafal Hanzl
    role: ProjectLeader
    slug: rafal-hanzl
    url: /people/rafal-hanzl/
  - name: Maureen Thomas
    role: Writer
- title: QueensGame
  year: '2022'
  source: Visual arts
  group_type: Visual arts
  url: https://runecast.net/queens-game/
  contributors:
  - name: Maureen Thomas
    role: Artist
  - name: Rafal Hanzl
    role: Artist
    slug: rafal-hanzl
    url: /people/rafal-hanzl/
- title: Medieval Akershus Castle - Interactive Museum Exhibit
  year: '2022'
  source: Exhibition
  group_type: Exhibition
  url: https://medievalcastle.extendedmedia.no
  contributors:
  - name: Rafal Hanzl
    role: ProjectLeader
    slug: rafal-hanzl
    url: /people/rafal-hanzl/
  - name: Maureen Thomas
    role: Writer
- title: Medieval Akershus Castle Model Installation
  year: '2022'
  source: Visual arts
  group_type: Visual arts
  url: https://extendedmedia.no/project/akershus-fortress-castle/
  contributors:
  - name: Rafal Hanzl
    role: Artist
    slug: rafal-hanzl
    url: /people/rafal-hanzl/
  - name: Maureen Thomas
    role: Consultant
- title: Songs from Akersborg Christmas 1363
  year: '2022'
  source: Performing arts
  group_type: Performing arts
  url: https://filmskolen.no/en/research/artistic-research/queens-game
  contributors:
  - name: Maureen Thomas
    role: ArtisticDirector
  - name: Rafal Hanzl
    role: Scenographer
    slug: rafal-hanzl
    url: /people/rafal-hanzl/
  - name: Kariina Gretere
    role: Actor
  - name: Idunn Snædis Agustsdottir
    role: SoundDesigner
  - name: Wenche Hellekås
    role: CostumeDesigner
  - name: Sindre Lie
    role: RoleOther
  - name: Johannes Skjeltorp-Borgaas
    role: RoleOther
source_mentions: []
summary: Researcher and digital artist specialising in immersive technologies, projection
  mapping, and cultural heritage digitisation. Honorary Professor at Kristiania University
  of Applied Sciences, Oslo, where he co-leads Phygital Lab. PhD in film arts (PWSFTviT
  Lodz, 2017), PKU artistic research fellowship (Den norske filmskolen, 2019). Background
  in VFX design, 3D modelling, and real-time visualisation using Unreal Engine and
  MetaHuman pipelines.
published: true
permalink: /people/rafal-hanzl/
wps:
- WP4
- WP6
---

Bio.
