---
layout: person
type: person
slug: ana-brotas
permalink: /people/ana-brotas/
name: Ana Brotas
title: Ana Brotas
position: Admin.sekretær/prosjektkoordinator
department: School of Arts, Design, and Media
institution: kristiania-university-college
institutions:
- kristiania-university-college
projects: []
roles:
- Member
urls:
  personal_website: https://anabrotas.com/
  institutional_website: ''
  github: ''
  linkedin: ''
  orcid: https://orcid.org/0009-0000-6192-6470
  nva: https://nva.sikt.no/research-profile/1799172
  youtube: ''
  facebook: ''
  mastodon: ''
  instagram: ''
aliases: []
tags:
- Pedagogy
- Communication
- Art
- Artistic research
search_keywords:
- Pedagogy
- Communication
- Art
- Artistic research
selected_works: []
source_mentions: []
summary: Ana Brotas has a background in public art, audiovisual communication, and
  pedagogy. Through her site-specific and socially engaged practice, she investigates
  how environments are understood, sensed, and imagined in times of ecological emergency.
  Previous projects were developed at places such as ARS-ID (PT), Silo Arte e Latitude
  Rural (BR), ROM for Art and Architecture (NO), V2_Lab for the Unstable Media (NL),
  and the European Capitals of Culture in Plovdiv (BG) and Veszprém (HU). Brotas holds
  a Master in Art and Public Space from the Oslo National Academy of the Arts (NO),
  for which she was awarded a bursary from the Calouste Gulbenkian Foundation (PT).
published: false
wps: []
---

Bio.
