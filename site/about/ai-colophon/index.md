...
- **Artwork.** The bubble emblem on the front page is redrawn every night by a [small script](https://github.com/MishMash-Norway/mishmash-web/blob/main/scripts/generate_daily_bubbles.py): a deterministic variation seeded by the date and the day's site activity (each small bubble is an upcoming event). It is generative in the algorithmic sense — no AI model is involved — and is labelled as generated where it appears.
