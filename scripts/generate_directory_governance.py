...
def render_person(name: str, slug: str, person: dict, institution_slugs: list[str]) -> str:
...
def write_directory(root: Path, persons: dict):
    ...
    for name, data in sorted(persons.items(), key=lambda x: x[0].lower()):
        ...
        out.write_text(render_person(name, person_slug, data, inst_slugs), encoding="utf-8")
