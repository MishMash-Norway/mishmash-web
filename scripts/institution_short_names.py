#!/usr/bin/env python3
"""Institution short names for portrait file names and generated labels.

`short_name` in each institution's index.md is the source of truth
(`load_institution_short_names`, `institution_abbrev`). `DEFAULT_SHORT_NAMES`
is only a fallback for institutions that lack one, and `suggest_short_name`
derives one from the name or slug when neither exists.
"""
